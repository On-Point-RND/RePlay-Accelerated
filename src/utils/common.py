...
if TORCH_AVAILABLE:
    from src.data.nn import PandasSequentialDataset, PolarsSequentialDataset, SequenceTokenizer
...
def save_to_src(obj: SavableObject, path: Union[str, Path]) -> None:
    """
    General function to save src models, splitters and tokenizer.
...
def load_from_src(path: Union[str, Path], **kwargs) -> SavableObject:
    """
    General function to load src models, splitters and tokenizer.

    :param path: Path to save the object.
    """
    path = Path(path).with_suffix(".src").resolve()
