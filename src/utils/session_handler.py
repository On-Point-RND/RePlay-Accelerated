...
def get_spark_session(
    ...
    :param core_count: Count of cores to execute, ``-1`` means using all available cores.
        If ``None`` then checking out environment variable ``src_SPARK_CORE_COUNT``,
    ...
    if os.environ.get("src_JAR_PATH"):  # pragma: no cover
        path_to_src_jar = os.environ.get("src_JAR_PATH")
    else:
        if pyspark_version.startswith("3.1"):  # pragma: no cover
            path_to_src_jar = (
                "https://repo1.maven.org/maven2/io/github/sb-ai-lab/src_2.12/3.1.3/src_2.12-3.1.3.jar"
            )
        elif pyspark_version.startswith(("3.2", "3.3")):  # pragma: no cover
            path_to_src_jar = "https://repo1.maven.org/maven2/io/github/sb-ai-lab/src_2.12/3.2.0_als_metrics/src_2.12-3.2.0_als_metrics.jar"
        elif pyspark_version.startswith("3.4"):  # pragma: no cover
            path_to_src_jar = "https://repo1.maven.org/maven2/io/github/sb-ai-lab/src_after_fix_2.12/0.1/src_after_fix_2.12-0.1.jar"
        else:  # pragma: no cover
            path_to_src_jar = (
                "https://repo1.maven.org/maven2/io/github/sb-ai-lab/src_2.12/3.1.3/src_2.12-3.1.3.jar"
            )
            logging.warning(
                "src ALS model support only spark 3.1-3.4 versions! src will use "
                "'https://repo1.maven.org/maven2/io/github/sb-ai-lab/src_2.12/3.1.3/src_2.12-3.1.3.jar' "
                "in 'spark.jars' property."
            )

    if core_count is None:  # checking out env variable
        core_count = int(os.environ.get("src_SPARK_CORE_COUNT", "-1"))
    if spark_memory is None:
        env_var = os.environ.get("src_SPARK_MEMORY")
    ...
    spark_session_builder = (
        ...
        .config("spark.jars", path_to_src_jar)
