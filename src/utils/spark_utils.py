...
def get_top_k(
    ...
    >>> from src.utils.session_handler import State
    >>> spark = State().session
    >>> log = (
    ...    spark
...
def multiply_scala_udf(scalar, vector):
    ...
    _f = sc._jvm.org.apache.spark.src.utils.ScalaPySparkUDFs.multiplyUDF()
...
def get_stats(log: SparkDataFrame, group_by: str = "user_id", target_column: str = "relevance") -> SparkDataFrame:
    ...
    >>> from src.utils.session_handler import get_spark_session, State
...
def sample_top_k_recs(pairs: SparkDataFrame, k: int, seed: Optional[int] = None):
    ...
    and could use `predict_pairs` method of src models to score them.
    After that we could select top K most relevant items for each user
    with `src.utils.spark_utils.get_top_k_recs` or sample them with
    probabilities proportional to their relevance score
    with `src.utils.spark_utils.sample_top_k_recs` to get more diverse recommendations.
